...
//                     resolve({ id: result.insertId, imageName: fileName });
//                 }
//             );
//         });
//     });
...
//     db.query('SELECT id, image_name, image_url FROM images ORDER BY uploaded_at DESC', (err, results) => {
//         if (err) {
...
// app.get('/api/show_one/:id', (req, res) => {
...
//     db.query('SELECT id, image_name, image_url FROM images WHERE id = ?', [id], (err, results) => {
//         if (err) {
//             console.error('Error fetching image:', err);
//             return res.status(500).send('Failed to fetch image.');
//         }

//         // If no image found for the provided ID
//         if (results.length === 0) {
//             return res.status(404).json({ message: 'Image not found.' });
//         }

//         // Prepare image data and send it as JSON
...
// // Delete a single photo and archive it in deleted_images_tb
...
//     // Fetch the photo before deletion
//     db.query('SELECT * FROM images WHERE id = ?', [id], (selectError, results) => {
//         if (selectError) {
...
//         // Insert the photo into deleted_images_tb
//         const insertQuery = `
//             INSERT INTO deleted_images_tb (id, imageName, imageUrl, deletedAt) 
//             VALUES (?, ?, ?, ?)
//         `;
//         db.query(
//             insertQuery,
//             [photo.id, photo.image_name, photo.image_url, deletedAt],
//             (insertError) => {
//                 if (insertError) {
//                     console.error('Error archiving deleted photo:', insertError);
//                     return res.status(500).json({ message: 'Failed to archive deleted photo.' });
//                 }

//                 // Delete the photo from images
//                 db.query('DELETE FROM images WHERE id = ?', [id], (deleteError, deleteResults) => {
//                     if (deleteError) {
...
// // Delete all photos and archive them in deleted_images_tb
...
//         if (results.length === 0) {
//             return res.status(200).json({ message: 'No photos to delete.' });
//         }

//         const deletedAt = new Date(); // Current timestamp
//         const deletedPhotos = results.map(photo => [photo.id, photo.image_name, photo.image_url, deletedAt]);
...
//             if (insertError) {
//                 console.error('Error archiving all deleted photos:', insertError);
//                 return res.status(500).json({ message: 'Failed to archive deleted photos.' });
//             }

//             // Truncate the images table
//             db.query('TRUNCATE TABLE images', (truncateError) => {
//                 if (truncateError) {
//                     console.error('Error truncating images table:', truncateError);
//                     return res.status(500).json({ message: 'Failed to truncate images table.' });
//                 }

//                 res.status(200).json({ message: 'All photos deleted and archived successfully.' });
//             });
//         });
//     });
// });
...
// // SELECT id, imageName, imageUrl, deletedAt, deleted_Id FROM deleted_images_tb WHERE 1

// app.get('/api/images_deleted_all', (req, res) => {
//     db.query('SELECT id, imageName, imageUrl, deletedAt, deleted_Id FROM deleted_images_tb ORDER BY deletedAt DESC', (err, results) => {
//         if (err) {
//             res.status(500).send('Failed to fetch images.');
//         } else {
//             const images = results.map((row) => ({
...
//     // Fetch the photo from deleted_images_tb before deletion
//     db.query('SELECT * FROM deleted_images_tb WHERE id = ?', [id], (selectError, results) => {
//         if (selectError) {
...
//         const insertQuery = `
//             INSERT INTO images (id, image_name, image_url) 
//             VALUES (?, ?, ?)
//         `;
//         db.query(
//             insertQuery,
//             [photo.id, photo.imageName, photo.imageUrl],
//             (insertError) => {
//                 if (insertError) {
...
//                 db.query('DELETE FROM deleted_images_tb WHERE id = ?', [id], (deleteError, deleteResults) => {
//                     if (deleteError) {
...
//     db.query('DELETE FROM deleted_images_tb WHERE id = ?', [id], (error, result) => {
//         if (error) {
...
function verifyEmail1(req, res, next) {
    ...
    req.email = email; // Save the email for further use
    next();
}
